...
UINT8 CommandMapper::AdjustTL(UINT8 tl) {
    ...
    // v2.6: Use v2.3's multiplication method (×2.5) to reduce volume to 40%
    // This is specifically for Super_Dodge_Ball which needs more volume reduction
    tl = tl & 0x7F;  // Mask bit 7 to ensure 7-bit value
    UINT16 adjusted = (tl * 5) / 2;  // Multiply by 2.5 (5/2)
